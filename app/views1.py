...
#@app.route is also known as a decorator. You decorated it with badges, or extra instructions

#@app.route("/", methods =["GET", "POST"])
#@app.route("/index", methods = ["GET", "POST"]) #url endpoints are really just functions in Python
def index():
	sentence = ""
	if request.method == "POST":
		sentence = request.param["sentence"]
	return render_template("index1.html", param = sentence)
